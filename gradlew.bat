...
@rem @rem   https://www.apache.org/licenses/LICENSE-2.0  @rem
...
set APP_HOME=%DIRNAME% @rem Resolve any "." and ".." in
APP_HOME to make it shorter. for %
...
if "%ERRORLEVEL%" == "0"
goto execute ERROR: JAVA_HOME is not set and no 'java'
...
echo.
echo Please set the JAVA_HOME variable in your environment to match the
echo location of your Java installation. goto fail:execute
@rem Setup the command line set 
CLASSPATH=%APP_HOME%\gradle\wrapper\gradle-wrapper.jar@rem Execute 
Gradle"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %GRADLE_OPTS% "-Dorg.gradle.appname=%APP_BASE_NAME%" -classpath "%CLASSPATH%" org.gradle.wrapper.GradleWrapperMain %*:end @rem End local 
scope for the variables with windows NT shell
...
if not " " == "%GRADLE_EXIT_CONSOLE%"exit /b 1:mainEnd if
"%OS%"=="Windows_NT"
endlocal:omega
